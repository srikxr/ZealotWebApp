...
export default function TopBar ({ showNav, setShowNav}) {
    return (
        <div className={`fixed w-full h-16 flex justify-between items-center transition-all duration-[400ms] ${showNav ? "pl-56" : ""}`}>
            ...
            <Bars3CenterLeftIcon className="h-8 text-gray-700 cursor-pointer" onClick={() => setShowNav(!showNav)} />
            </div>
                ...
                <Menu as="div" className="relative inline-block text-left">
                    <div>
                        <Menu.Button className="inline-flex w-full justify-center items-center">
                            <picture>
                                <img 
                                    src="https://zealot.s3.us-west-2.amazonaws.com/manSmiling.jpg"
                                    className='rounded-full h-8 md:mr-4 border-2 border-white shadow-sm'
                                    alt="profile picture"
                                /> 
                            </picture>
                            <span className="hidden md:block font-medium text-gray-700">Sam</span>
